...
class KeepsakesScanHandler(ShelfScanner):
    ...
    def get_or_create_keepsake(self, info: dict):
        if not info["directory"] in self.batch_lookup:
            keepsake = db.op.get_keepsake_by_directory(
                directory=info["directory"], load_files=False
            )
            if not keepsake:
                keepsake = db.op.create_keepsake(directory=info["directory"])
                db.op.add_keepsake_to_shelf(
                    shelf_id=self.shelf.id, keepsake_id=keepsake.id
                )
        ...
        for info in self.file_info_lookup["image"]:
            ...
            if progress_count % 500 == 0:
                db.op.update_job(
                    ...
                    message=f"Organize keepsake image {progress_count} out of {len(self.file_info_lookup['image'])}",
                )
    ...
    def organize_videos(self):
        ...
        for info in self.file_info_lookup["video"]:
            ...
            if progress_count % 500 == 0:
                db.op.update_job(
                    job_id=self.scope.job_id,
                    message=f"Organize keepsake video {progress_count} out of {len(self.file_info_lookup['video'])}",
                )
            keepsake = self.get_or_create_keepsake(info=info)
            keepsake_video_file = db.op.get_keepsake_video_file(
