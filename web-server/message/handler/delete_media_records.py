...
def handle(scope:JobMediaScope):
    ...
    if scope.is_shelf():
        ...
        db.op.delete_show_records(ticket=ticket,movie_id=scope.target_id)
    elif scope.is_season():
        db.op.delete_show_season_records(ticket=ticket,movie_id=scope.target_id)
    elif scope.is_episode():
        db.op.delete_show_episode_records(ticket=ticket,movie_id=scope.target_id)