...
def handle(scope: JobMediaScope):
    ...
    if results:
        ...
        db.op.update_job(
            job_id=scope.job_id,
            message=f"Purged {len(results)} orphaned records\n{details}",
        )
    ...
    if results:
        ...
        db.op.update_job(
            job_id=scope.job_id,
            message=f"Purged {len(results)} metadata file records\n{details}",
        )
    ...
    if results:
        ...
        db.op.update_job(
            job_id=scope.job_id,
            message=f"Purged {len(results)} image file records\n{details}",
        )
    ...
    if results:
        ...
        db.op.update_job(
            job_id=scope.job_id,
            message=f"Purged {len(results)} video file records\n{details}",
        )
