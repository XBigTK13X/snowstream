...
def auth_required(router):
    ...
    def get_stream_source_list(
        ...
        return db.op.get_stream_source_list(ticket=auth_user.ticket)
    ...
    def save_stream_source(
        ...
        if not auth_user.is_admin():
            return None
        return db.op.upsert_stream_source(
            ticket=auth_user.ticket, stream_source=stream_source
        )
    ...
    def get_stream_source(
        ...
        if not auth_user.ticket.is_allowed(stream_source_id=stream_source_id):
            return None
        stream_source = db.op.get_stream_source_by_id(
            ticket=auth_user.ticket, stream_source_id=stream_source_id
        )
        ...
        for streamable in stream_source.streamables:
            if streamable.channel:
                ...
                for program in streamable.channel.programs:
                    if program.start_datetime <= now and program.stop_datetime >= now:
                        program.display_time = program.start_datetime.strftime(
                            "%I:%M %p"
                        )
                        ...
                        if program.start_datetime > now:
                            if (
                                soonest == None
                                or program.start_datetime < soonest.start_datetime
                            ):
                                soonest = program
                if soonest:
            ...
            group = (
                streamable.group_display
                if streamable.group_display
                else streamable.group
            )
            if group == None:
                group = "No Group"
        ...
        if stream_source.kind == "HdHomeRun" or stream_source.kind == "IptvM3u":
            stream_source.streamables = sorted(
                stream_source.streamables, key=lambda xx: xx.name
            )
            for group in stream_source.groups:
                stream_source.grouped_streamables[group].sort(
                    key=lambda xx: xx.name_display if xx.name_display else xx.name
                )
    ...
    def get_streamable_list(
        ...
        stream_sources = db.op.get_stream_source_list(
            ticket=auth_user.ticket, streamables=True
        )
        streamables = []
    ...
    @router.delete(
        "/channel/guide/source/{channel_guide_source_id}", tags=["Channel Guide"]
    )
    def delete_channel_guide_source(
        ...
        if not auth_user.is_admin():
            return None
        return db.op.delete_channel_guide_source_by_id(
            channel_guide_source_id=channel_guide_source_id
        )
    ...
    def get_channel_guide_source(
        ...
        source = db.op.get_channel_guide_source_by_id(
            channel_guide_source_id=channel_guide_source_id
        )
        if source.channels:
    ...
    def toggle_shelf_watched(
        ...
        if movie_shelf_id:
            is_watched = db.op.get_movie_shelf_watched(
                ticket=auth_user.ticket, shelf_id=movie_shelf_id
            )
        ...
        if show_shelf_id:
            is_watched = db.op.get_show_shelf_watched(
                ticket=auth_user.ticket, shelf_id=show_shelf_id
            )
    ...
    def set_shelf_watched(
        ...
        if movie_shelf_id:
            return db.op.set_movie_shelf_watched(
                ticket=auth_user.ticket, shelf_id=movie_shelf_id, is_watched=is_watched
            )
        if show_shelf_id:
            return db.op.set_show_shelf_watched(
                ticket=auth_user.ticket, shelf_id=show_shelf_id, is_watched=is_watched
            )
    ...
    def get_cached_text_by_query(
        ...
        if result == None:
            return {"content": f"No content cached under [{cache_key}]"}
    ...
    def get_movie_details(
        ...
        movie.watched = db.op.get_movie_watched(
            ticket=auth_user.ticket, movie_id=movie_id
        )
        movie.has_extras = False
        movie.has_versions = False

        for ii in range(0, len(movie.video_files)):
            movie.video_files[ii].info = json.loads(
                movie.video_files[ii].snowstream_info_json
            )
        ...
        movie.discussion_image_url, movie.discussion_url = util.search_to_base64_qrcode(
            search_query
        )
        return movie
    ...
    def set_movie_watched(
        ...
        return db.op.set_movie_watched(
            ticket=auth_user.ticket, movie_id=movie_id, is_watched=is_watched
        )
    ...
    def toggle_movie_watched(
        ...
        is_watched = db.op.get_movie_watched(ticket=auth_user.ticket, movie_id=movie_id)
        db.op.set_movie_watched(
            ticket=auth_user.ticket, movie_id=movie_id, is_watched=not is_watched
        )
        return not is_watched
    ...
    def increase_movie_watch_count(
        ...
        return db.op.increase_movie_watch_count(
            ticket=auth_user.ticket, movie_id=movie_id
        )
    ...
    def set_movie_watch_progress(
        ...
        return db.op.set_movie_watch_progress(
            ticket=auth_user.ticket, watch_progress=watch_progress
        )
    ...
    def get_show_list(
        ...
        return db.op.get_show_list_by_shelf(
            ticket=auth_user.ticket, shelf_id=shelf_id, show_playlisted=show_playlisted
        )
    ...
    def set_show_watched(
        ...
        return db.op.set_show_watched(
            ticket=auth_user.ticket, show_id=show_id, is_watched=is_watched
        )
    ...
    def toggle_show_watched(
        ...
        is_watched = db.op.get_show_watched(ticket=auth_user.ticket, show_id=show_id)
        db.op.set_show_watched(
            ticket=auth_user.ticket, show_id=show_id, is_watched=not is_watched
        )
        return not is_watched
    ...
    def get_show_season_list(
        ...
        return db.op.get_show_season_list_by_show_id(
            ticket=auth_user.ticket, show_id=show_id
        )
    ...
    def set_show_season_watched(
        ...
        return db.op.set_show_season_watched(
            ticket=auth_user.ticket, season_id=season_id, is_watched=is_watched
        )
    ...
    def toggle_show_season_watched(
        ...
        is_watched = db.op.get_show_season_watched(
            ticket=auth_user.ticket, season_id=season_id
        )
        db.op.set_show_season_watched(
            ticket=auth_user.ticket, season_id=season_id, is_watched=not is_watched
        )
        return not is_watched
    ...
    def set_show_episode_watched(
        ...
        return db.op.set_show_episode_watched(
            ticket=auth_user.ticket, episode_id=episode_id, is_watched=is_watched
        )
    ...
    def toggle_show_episode_watched(
        ...
        episode = db.op.get_show_episode_by_id(
            ticket=auth_user.ticket, episode_id=episode_id
        )
    ...
    def increase_show_episode_watch_count(
        ...
        return db.op.increase_show_episode_watch_count(
            ticket=auth_user.ticket, show_episode_id=show_episode_id
        )
    ...
    def get_show_episode_details(
        ...
        for ii in range(0, len(episode.video_files)):
            episode.video_files[ii].info = json.loads(
                episode.video_files[ii].snowstream_info_json
            )
        ...
        episode.discussion_image_url, episode.discussion_url = (
            util.search_to_base64_qrcode(search_query)
        )
        return episode
    ...
    def set_show_episode_watch_progress(
        ...
        return db.op.set_show_episode_watch_progress(
            ticket=auth_user.ticket, watch_progress=watch_progress
        )
    ...
    def get_keepsake(
        ...
        current_keepsake = db.op.get_keepsake_by_directory(
            directory=absolute_subdirectory
        )
        ...
        if current_keepsake:
            ...
            videos.sort(key=lambda xx: xx.name.lower())
            images.sort(key=lambda xx: xx.name.lower())
        ...
        directories.sort(key=lambda xx: xx["display"].lower())
    ...
    async def stream_transcode(transcode_session_id: int):
        session = db.op.get_transcode_session_by_id(
    ...
    def get_playing_queue(
        ...
        if show_season_id:
            ...
            item = db.op.get_show_season_by_id(
                ticket=auth_user.ticket, season_id=show_season_id
            )
        if tag_id:
            kind = "playlist"
            kind_id = tag_id
        ...
        if not kind and source:
            ...
            if kind == "show_season":
                item = db.op.get_show_season_by_id(
                    ticket=auth_user.ticket, season_id=kind_id
                )
            if kind == "tag":
                kind = "playlist"
                item = db.op.get_tag_by_id(tag_id=kind_id)
        ...
        return {"queue": queue, "kind": kind, "kind_id": kind_id, "item": item}
    ...
    def update_playing_queue(
        ...
        db.op.update_playing_queue(
            ticket=auth_user.ticket, source=source, progress=progress
        )
...
def no_auth_required(router):
    ...
    @router.get(
        "/streamable.m3u", response_class=PlainTextResponse, tags=["Unauthed Video"]
    )
    def get_streamable_m3u():
        return db.op.get_cached_text_by_key(key=cache.key.STREAMABLE_M3U)

    @router.get(
        "/streamable.xml", response_class=PlainTextResponse, tags=["Unauthed Video"]
    )
    ...
    def get_transcode_playlist(transcode_session_id: int):
        playlist_content = transcode_sessions.get_playlist_content(
            transcode_session_id=transcode_session_id
        )
    ...
    async def webhook(kind: str, request: Request):
        ...
        if is_movie or is_show:
            ...
            message.write.send(
                job_id=scan_job.id, kind="scan_shelves_content", input=input
            )
        else:
